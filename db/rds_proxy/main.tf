locals {
  ...
  role_name   = coalesce(var.iam_role_name, "${var.name}-role")
  policy_name = coalesce(var.iam_policy_name, "${var.name}-policy")
}
...
resource "aws_db_proxy" "this" {
  ...
  dynamic "auth" {
    ...
    content {
      ...
      # client_password_auth_type = try(auth.value.client_password_auth_type, "POSTGRES_MD5") # need terraform aws provider - v5.1.0
