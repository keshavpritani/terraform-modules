resource "aws_networkfirewall_firewall" "this" {
  ...
  dynamic "subnet_mapping" {
    for_each = toset([
      for az, ids in var.subnet_mapping : ids[0]
    ])
  ...
  lifecycle {
    precondition {
      condition     = length(keys(local.az_rt_ids)) == length(toset(flatten(values(local.az_rt_ids))))
      error_message = "Every az should have there own route table"
    }
    precondition {
      condition     = sort(keys(local.az_rt_ids)) == sort(keys(var.subnet_mapping))
      error_message = "NF Subnet AZs are not matching with Bussiness Subnet's AZs"
    }
  }
}

resource "aws_networkfirewall_rule_group" "suricata_stateful_group" {
  count = var.create && length(var.suricata_stateful_rule_group) > 0 ? length(var.suricata_stateful_rule_group) : 0
  type  = "STATEFUL"

  name        = var.suricata_stateful_rule_group[count.index]["name"]
  description = try(var.suricata_stateful_rule_group[count.index]["description"], null)
  capacity    = var.suricata_stateful_rule_group[count.index]["capacity"]

  rule_group {
    rules_source {
      rules_string = try(var.suricata_stateful_rule_group[count.index]["rules_string"], try(file(var.suricata_stateful_rule_group[count.index]["rules_file"]), null))
    }
...
resource "aws_networkfirewall_rule_group" "domain_stateful_group" {
  count = var.create && length(var.domain_stateful_rule_group) > 0 ? length(var.domain_stateful_rule_group) : 0
  type  = "STATEFUL"

  name        = var.domain_stateful_rule_group[count.index]["name"]
  description = try(var.domain_stateful_rule_group[count.index]["description"], null)
...
resource "aws_networkfirewall_rule_group" "fivetuple_stateful_group" {
  count = var.create && length(var.fivetuple_stateful_rule_group) > 0 ? length(var.fivetuple_stateful_rule_group) : 0
  type  = "STATEFUL"

  name        = var.fivetuple_stateful_rule_group[count.index]["name"]
  description = try(var.fivetuple_stateful_rule_group[count.index]["description"], null)
...
resource "aws_networkfirewall_rule_group" "stateless_group" {
  count = var.create && length(var.stateless_rule_group) > 0 ? length(var.stateless_rule_group) : 0
  type  = "STATELESS"

  name        = var.stateless_rule_group[count.index]["name"]
  description = try(var.stateless_rule_group[count.index]["description"], null)
...
resource "aws_cloudwatch_log_group" "nfw" {
  for_each = { for k, v in try(var.logging_config, {}) : k => v if var.create }
...
resource "aws_networkfirewall_logging_configuration" "this" {
  count        = var.create && try(length(var.logging_config), 0) > 0 ? 1 : 0
...
data "aws_route_table" "az_rts" {
  for_each  = var.business_public_subnets_az_wise
  subnet_id = each.value[0]
}

module "routes" {
  source         = "../route_tables/routes"
  for_each       = { for k, v in local.az_rt_ids : k => v if var.create }
  route_table_id = each.value
  ipv4_routes = [
    {
      cidr_block      = "0.0.0.0/0",
      vpc_endpoint_id = lookup(local.vpn_endpoint_az_wize, each.key, null)
    },
  ]
}


data "aws_subnet" "subents" {
  for_each = toset(flatten(values(var.business_public_subnets_az_wise)))
  id       = each.value
}

module "nf_routes" {
  source         = "../route_tables/routes"
  for_each       = { for id, value in data.aws_subnet.subents : id => value if var.create }
  route_table_id = var.nf_incoming_rt_id
  ipv4_routes = [
    {
      cidr_block      = each.value.cidr_block,
      vpc_endpoint_id = lookup(local.vpn_endpoint_az_wize, each.value.availability_zone, null)
    },
  ]
}
